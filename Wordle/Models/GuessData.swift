...
struct GuessData {
    ...
    mutating func restartGame() {
        targetWord = answerWordList.randomElement()!
        currentGuess = []
        pastGuesses = []
        pastGuessesColor = Array(repeating: Array(repeating: Color.white, count: 5), count: 6)
        numberOfGuesses = 0
        gameOver = false
        letterColors = Dictionary(uniqueKeysWithValues: "ABCDEFGHIJKLMNOPQRSTUVWXYZ".map { (String($0), Color.lightestGray) })
    }
